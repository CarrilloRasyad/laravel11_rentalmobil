<div class="container-fluid pt-4 px-4">
    <div class="row g-4">
        <div class="col-sm-12 col-xl-12">
            <div class="bg-light rounded h-100 p-4">
                @if (session()->has('success'))
                    <div class="alert alert-success" role="alert">
                        {{ session('success') }}
                    </div>
                @endif
                <h6 class="mb-4">Data Laporan Transaksi</h6>
                    <div class="row align-items-center">
                        <div class="col-md-4 pe-0">
                            <input type="date" wire:model="tanggal1" class="form-control" placeholder="Tanggal">
                        </div>
                        <div class="col-md-auto px-2 d-flex align-items-center justify-content-center" style="height: 38px;">
                            To
                        </div>
                        <div class="col-md-4 ps-0">
                            <input type="date" wire:model="tanggal2" class="form-control" placeholder="S/d Tanggal">
                        </div>
                        <div class="col-md-2">
                            <button class="btn btn-xl btn-primary h-100" wire:click="cari">Cari</button>
                        </div>
                    </div>
                <table class="table">
                    <thead>
                        <tr>
                            <th scope="col">No</th>
                            {{-- <th scope="col">No.Polisi</th> --}}
                            <th scope="col">Nama Pemesan</th>
                            <th scope="col">Alamat</th>
                            <th>Waktu</th>
                            <th>Tanggal Pesanan</th>
                            <th>Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($transaksi as $data)
                        <tr>
                            <th scope="row">{{$loop->iteration}}</th>
                            {{-- <td>{{$data->mobil->nopolisi}}</td> --}}
                            <td>{{$data->nama}}</td>
                            <td>{{$data->alamat}}</td>
                            <td>{{$data->waktu}} hari</td>
                            <td>{{$data->tgl_pesanan}}</td>
                            <td>Rp {{$data->total}}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="8">Data Laporan Transaksi Tidak Ada!</td>
                        </tr> 
                        @endforelse
                    </tbody>
                </table>
                {{$transaksi->links()}}
                <button class="btn btn-primary" wire:click="exportpdf">Export PDF</button>
            </div>
        </div>
    </div>
</div>   
